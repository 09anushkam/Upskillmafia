...
3. npm run dev (to run vite app)  
4. Delete useless files/folders  
5. make changes in code (App.jsx,main.jsx)  
6. npm run dev (to run vite app)  
...
04. const [counter,setCounter]=useState(5);  
...
05. hooks eg:  useState,useCallBack,useEffect,useRef,useNavigate  

06. props - reusablity of Components  

07. onClick expects function now the value that func is returning `onClick={()=>setColor("lavender")}`  

08. useState updates state of variable  
const [variable, setVariable] = useState(initial value);  

09. useEffect avoids clicking rendering something instead it directly renders that something on page  
life cycle of react component (mount,unmount,update)  
then try to understand useEffect  
useEffect(callback,dependency);  
dependency term is not clear  
useEffect(() => {  
    return () => {  
...
10. useRef allows you to persist values across renders without causing the component to re-render when the value changes. It provides a way to access and interact with DOM elements directly.  
